...
改修依頼のたびに貼り付けていた【改修時の留意事項】は、以下として**常に適用**する。今後の依頼は
**管理番号**と**改修内容**だけでよい（留意事項の再掲は不要。今回だけ方針を変えたいときのみ都度明記する）。

1. 優先順位をつけて改修する。1回の処理で品質が下がりそうなら分割し、**実施した項目番号を結果に明記**する（積み残しは次回へ）。
2. レイアウト改修は**デザイン性・ユーザビリティ・見やすさ・操作性**を重視する。
3. スパゲッティコードを避ける。**他サービス・他プログラム・連携先への影響**（2システムで複製し合うコードのペア含む）を
   全体設計として考慮し、連携先の改修要否まで判断してから進める。
4. 依頼内容に**不明点があれば必ず確認**する。憶測で作業しない。
5. 完了後は `docker compose up -d --build` → `git push` を実施する。
6. 開発後は**必ず要件充足をテストで確認**する。ただし**本番メールを送信しない**手段で行う
   （承認等の操作でテストメールが実送信されるため）。自動テスト（pytest）は `MAIL_BACKEND=console` 既定で実送信ゼロ。
