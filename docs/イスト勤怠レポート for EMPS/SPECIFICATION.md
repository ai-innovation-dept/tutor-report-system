...
- **ユーザーのコピー**（`POST /api/w/users/copy`・`services/user_service.copy_user`）: 入力は `source_user_id`・`display_name`・`email` のみ。**氏名・メールはどちらも重複エラー**（氏名＝未削除ユーザー内で一致、メール＝一意制約のため削除済みを含め既存と一致で 409）。ロール（複数ロール含む `roles`）・利用システム（`allowed_systems`）・学校の承認スキップ（`skip_parent_approval`）を**コピー元から複製**し、**招待メールは送らず直接作成**（`password_hash`＝初期パスワード `Passw0rd!`・`must_change_password=True`。`user_no` はコピー元の主ロール帯で自動採番＝`create_initial_user` と同規約。電話番号など個人情報は複製しない）。管理責任者（`admin_chief`）を含むユーザーのコピーは管理責任者のみ（招待・無効化と同じ職務分掌で 403）。
- **契約のコピー**（バックエンドは既存の `POST /api/w/contracts` を利用＝**専用APIなし**）: 画面のコピーボタンが契約編集ドロワーを**作成モードでコピー元の全項目をプレフィル**して開く（就業場所・期別委託業務・コマ設定・副業務・任意項目列・表示フラグ・契約期間 等）。**講師・学校のみ選び直して**登録し、契約番号は作成順で新しく自動発番される。同一講師×学校は既存の重複チェックで 409。フロントは `renderForm` の編集判定を `editingId` 基準へ変更し、`copyMode`／`copySource`（未使用コマ設定の `period_slots` 引き継ぎ用）を追加している。
