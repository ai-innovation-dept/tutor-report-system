...
担当業務（`task_minutes_N`）の対象月の合計が、契約の月分固定（`workload_cases` の該当ケース）を超過する報告は、学校確認の前に**事務の事前確認**を挟む（`engine.exceeds_monthly_limit` が提出時に判定し、`submit` の遷移先を `awaiting_school` → `awaiting_office_precheck` に差し替える）。

```
draft --submit(講師, 超過判定あり)--> awaiting_office_precheck
...
> 判定対象は担当業務（`task_minutes_N`）のみ。週コマ・副業務・ケース未登録の業務は対象外。学校承認スキップ校は超過時でも事前確認を挟まず通常スキップフロー（事務確認1回）になる（スキップ判定が優先）。
...
|----|---------|----------|:----:|
| `draft` | 下書き | tutor | |
| `awaiting_office_precheck` | 事務事前確認待ち（超過時のみ） | office | |
