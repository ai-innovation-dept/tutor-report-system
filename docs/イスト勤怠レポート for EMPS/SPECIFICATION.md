...
| PATCH | `/api/w/reports/{id}/school-requests` | school | **要望連絡事項（学校）のみ**の保存（自校・学校確認待ちのみ。対象月は問わない。明細・他メタは受け取らない・通知なし） |
...
> 報告書（明細）の編集ができるのは**本人（講師）＝ `PATCH /{id}` と 事務 ＝ `PATCH /{id}/office-edit` のみ**。営業は承認／差戻しのみで編集はできない。学校は `PATCH /{id}/school-requests` で**要望連絡事項（学校）の1項目だけ**を書ける（§9「要望連絡事項の3欄」参照）。
...
- 講師の報告書には、期別設定を **要望連絡事項（運営）**「【前期】業務名：（月　3,000　分固定　：　週15コマ）[2026年04月01日　～　2026年08月31日]」形式＋契約期間、**委託業務（契約より）**（前期・後期の名称・委託業務ID・個別契約ID→`meta.task_reference` へスナップショット）、**スケジュール欄**（期別コマ設定「【前期】① 8:30〜9:20、…」）として反映する（いずれも契約由来＝講師変更不可・サーバ側ロック。要望連絡事項等は両期を併記するが、明細の担当業務列・コマ設定の適用は入力タイミングの期のみ＝§9）
...
### 要望連絡事項の3欄（運営／講師／学校・改修依頼 202607211716-②）

明細の下の「要望連絡事項」は**運営・講師・学校の3欄**に分かれ、**入力・編集できるのは担当ロールのみ**（他ロールからは読取専用）。3欄とも `form_data.meta` に保存する（DBスキーマ変更なし）。講師フォーム・参照画面（report_view）・PDF のいずれにも3欄そろって表示する。

| 欄 | meta キー | 入力する画面・ロール | 他ロールからの見え方 |
|---|---|---|---|
| 要望連絡事項（運営） | `requests` | 契約管理（経理・事務）で設定した**担当業務の期別設定＋契約期間から自動生成**（`workloadRequestsText`。自由記述欄は持たない） | 読取専用（契約由来のためサーバ側でも講師の保存をブロック） |
| 要望連絡事項（講師） | `requests_tutor` | 講師が業務連絡表（`/tutor/reports`）で入力・編集（下書き／差戻し中＝報告書が編集できる間） | 読取専用 |
| 要望連絡事項（学校） | `requests_school` | 学校が承認管理の「報告書を確認」（report_view）で入力・編集。**学校確認待ち（`awaiting_school`）の間のみ**・対象月は問わない（案B＝月非依存） | 読取専用 |

- 学校欄の保存は専用API **`PATCH /api/w/reports/{id}/school-requests`**（学校ロール・自校・`awaiting_school` のみ。明細や他のメタ項目は受け取らないため、学校が講師の記入内容を書き換えることはできない）。承認・差戻し・差戻し要求の許可を押したときは、**ボールを離す前に未保存の入力を自動保存**する。
- 講師の保存（`PATCH /api/w/reports/{id}`）は meta を丸ごと組み立て直して送るため、サーバー側で `requests_school` を保存済みの値へ固定する（`_OTHER_ROLE_META_KEYS`）。新規作成時に講師が送った `requests_school` は捨てる（学校欄のなりすまし防止）。
- 学校欄の保存では通知メールを送らない（内容は承認・差戻しの既存メールと画面で到達する）。
- **学校確認スキップ校**（学校ユーザーの `skip_parent_approval`）は報告書が `awaiting_school` を通らないため、学校欄は使えない（仕様上の帰結。スキップ＝学校が確認しない運用のため）。

### 記入コピー（講師フォーム）

報告書一覧の上部（フォーム外の右端）に2つのコピーボタンを並べて表示する。いずれも編集可能な報告書（下書き・差戻し中。**過去月も可**＝改修 202607211716・案B）でのみ有効（読取専用時は無効化、提出専用ページでは非表示）。
...
- **先月の記入分をコピー**（2026-07-08）: 選択中の学校の**先月の報告書**の明細（種別を含む・ステータス不問）を当月フォームへ反映する。
  ...
  - メタ項目（要望連絡事項3欄・定期代等）はコピー対象外。既に入力中の明細がある場合は置き換えの確認ポップアップを表示する。
