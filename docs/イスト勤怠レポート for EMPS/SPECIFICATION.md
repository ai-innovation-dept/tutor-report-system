...
- コマ設定（担当時限の時間割・最大10コマ）は**期ごとに設定**する（`workload_cases[].slots`）。番号は①から詰めて入力（空き番号不可）・**コマ番号は時間順でなくてもよい**（例: ⑤に①より早い朝の時間帯を追加可・2026-07-16修正）。どの2コマも時間帯の重なりは不可（検証は `_validate_slot_list`＝画面・APIで共通のペア重なり判定）。自動計算は選択コマを**開始時刻順に並べ替えて**行う（業務開始＝最早コマの開始・休憩＝時刻順の隙間。`slotSelectionMetrics`）。旧形式の契約単位 `period_slots` は読込互換のみ（画面保存で期別へ一本化）。休憩時間列を非表示にした契約とは併用不可
