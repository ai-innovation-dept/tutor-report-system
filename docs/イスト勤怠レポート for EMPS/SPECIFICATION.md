...
- **担当業務は前期・後期の2本必須**（2026-07-16改修。データ上は `task_name_1`=前期／`task_name_2`=後期。旧仕様①〜③の `task_name_3` 列は残るが新規保存では未使用）: 各期に委託業務名（必須）・委託業務ID・個別契約ID＋**期別設定**（`workload_cases`。`task_index` 1=前期／2=後期）として月時間（分）・週コマ（任意）・**適用期間（開始・終了。必須・前期と後期の重複不可）**・**期別コマ設定**（`slots`）を持つ
- 副業務①〜⑤（`sub_task_name_1..5`・任意）: 業務名・委託業務ID（`sub_task_id`）・個別契約ID（`sub_contract_id`）。業務名があるもののみ報告書に「業務名（分）」列を生成
- コマ設定（担当時限の時間割・最大10コマ）は**期ごとに設定**する（`workload_cases[].slots`）。旧形式の契約単位 `period_slots` は読込互換のみ（画面保存で期別へ一本化）。休憩時間列を非表示にした契約とは併用不可
- 採点欄: `採点を追加する`（`scoring_enabled`）＋ 項目名（`scoring_label`・既定「採点」）・単位（`scoring_unit`・既定「回」）・委託業務ID・個別契約ID（有効時のみ報告書末尾に「採点（回）」列を生成）
- 前期・後期の必須検証（名称・適用期間・重複なし）は `schemas/contracts.term_payload_errors` に集約し、画面保存（POST/PATCH）・CSV取込で共用する。旧形式の契約（期間なし）も他フィールドのみの部分更新は通る（新仕様は契約を編集した時点から適用）
- 講師の報告書には、期別設定を **要望連絡事項**「【前期】業務名：（月　3,000　分固定　：　週15コマ）[2026年04月01日　～　2026年08月31日]」形式＋契約期間、**委託業務（契約より）**（前期・後期の名称・委託業務ID・個別契約ID→`meta.task_reference` へスナップショット）、**スケジュール欄**（期別コマ設定「【前期】① 8:30〜9:20、…」）として反映する（いずれも契約由来＝講師変更不可・サーバ側ロック）
...
- 担当業務列は前期・後期の期別（2026-07-16改修）: `担当業務(前期)名／ID／個別契約ID`＋`月時間(分)(前期)`・`週コマ(前期)`・`適用開始(前期)`・`適用終了(前期)`（後期も同様）。名称・適用期間は必須（画面保存と同じ `term_payload_errors` で検証）。
- 表示項目フラグ・期別コマ設定（`slots`）はCSV対象外（取込・upsert時も既存設定を `task_index` で引き継いで保持）。
...
| 動的 | 担当業務（**対象月の適用期間に該当する期のみ**）「業務名（分）」 | task_minutes_1（前期）/ task_minutes_2（後期） | number（合計対象） |
...
- **担当業務列は対象月の期のみ生成**する（`build_column_definition(profile, target_month)`。`GET /api/w/contracts/for-tutor?target_month=YYYY-MM`・省略時は現在月）。通常の月は1列。**期の切替が月の途中にある月は前期・後期の2列**（行の日付でどちらへ入力するか自動判定）。どの期にも該当しない月（期間の隙間・旧データ）は入力不能にならないよう登録済みの担当業務すべてへフォールバック。
- 期別コマ設定（`workload_cases[].slots`）がある契約は、**行の日付がどちらの適用期間内か**でその期の時間割を適用する（担当時限の選択肢・自動計算とも。共通コア `termSlotsForContext`）。日付を期跨ぎで変更すると担当業務（分）・休憩・業務開始〜終了時間を新しい期で再計算する。
...
- コマ設定は `GET /api/w/contracts`（office権限）から講師×学校で該当契約を引いて参照する（モーダル初回表示時に一度だけ取得）。期別コマ設定（`workload_cases[].slots`）は講師フォームと同じく**行の日付から期を判定**して適用し、旧形式（契約単位 `period_slots`）はフォールバック。契約が無い報告書は8:40固定ルール。
