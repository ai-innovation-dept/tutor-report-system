...
講師向けに、指導報告（勤務時間・日報・月報）の**提出締切＝対象月の翌月1日（年間固定・土日祝でも繰り延べない。改修 202607231903）**を2段階で通知する。
例: 2026年7月分→8月1日（土）、2026年8月分→9月1日（火）。実装は `backend/app/services/deadline_service.py` に集約。
※ 旧仕様（〜202607231903）は「翌月第一営業日」（`jpholiday`＋`BUSINESS_CLOSED_DAYS` で繰り延べ）だったが廃止（設定キーが .env に残っていても無視される）。
