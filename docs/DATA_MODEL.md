...
| grade_level | VARCHAR(2) | NULL | 学年区分（小/中/高）。教科の前に置く先頭項目。必須の強制は入力UI側 |
| grade_year | INTEGER | NULL | 学年数（小1〜6・中/高1〜3）。必須の強制は入力UI側 |
| subject | VARCHAR(100) | NULL | 教科（旧「科目」を流用・名称変更。必須の強制は入力UI側） |
| material_name | TEXT | NULL | (a) 使用教材／テキスト名（必須の強制は入力UI側） |
| content | TEXT | NOT NULL | (b) 何を指導したか／単元など（旧「指導内容」を流用） |
| learning_status | TEXT | NULL | (c) 学習状況／問題と対策（必須の強制は入力UI側） |
| homework_status | VARCHAR(1) | NULL | (d) 宿題／状況 A=ほぼ完ぺき / B=半分くらい / C=やらなかった |
| next_homework | TEXT | NULL | 次回までの宿題（必須の強制は入力UI側） |
| next_lesson_date | DATE | NULL | 次回の予定／指導日（任意） |
| next_lesson_start | TIME | NULL | 次回の指導開始時刻（任意） |
...
委託業務は **担当業務（前期・後期の2本必須。2026-07-16改修）** と **サブ業務（①〜⑤・任意）** の2区分に分かれ、報告書の列は「担当（入力タイミング＝今日基準で適用中の期の1列のみ）→サブ」の順に生成される（`task_minutes_N`（N=1:前期／2:後期） / `sub_minutes_N`）。
...
> **委託業務と採点の報告書反映**: 担当（前期/後期）・サブ①〜⑤の委託業務は常に「分のみ」（「業務名（分）」列）で、報告書では担当→サブの順に生成される。担当業務列は**入力タイミング（今日JST。過去月の報告書は月内へクランプ）基準で適用中の期の1列のみ**（期の切替が月の途中にある月でも1列。該当期が無い月・期間なしの旧データは全担当業務へフォールバック）。採点は `scoring_enabled=True` のときのみ報告書末尾に「{scoring_label}（{scoring_unit}）」列（既定「採点（回）」、1セル併記＝回数＋分数固定）を生成する。`show_*` フラグは契約からライブ反映され、報告書フォームの各欄／列の表示・非表示を制御する。動的列定義は `services/contract_form_service.build_column_definition(profile, target_month)`（期解決は `active_term_case`）が生成する。
...
### work_no_lesson_months（講師の「当月授業なし」申請・202607161140）

長期休業などで授業を行わない月の講師本人による申請（講師×月で1行・全契約対象・new側 migration 0018）。申請中の講師は学校の「契約講師全員の学校承認完了」通知（`school_progress_service`）の集計対象外になる。報告書の作成・提出は制限しない。

| カラム | 型 | 制約 | 説明 |
|--------|------|------|------|
| id | UUID | PK, default=uuid4 | ID |
| tutor_id | UUID | FK(users.id), INDEX | 申請した講師 |
| target_month | VARCHAR(7) | INDEX | 対象月（YYYY-MM）。`UNIQUE(tutor_id, target_month)` |
| created_at | TIMESTAMP WITH TZ | NOT NULL | 申請日時 |

### work_school_settings（学校の締め日通知設定・202607161140）

学校ユーザー単位の設定（ユーザ管理の学校詳細で編集・new側 migration 0018）。

| カラム | 型 | 制約 | 説明 |
|--------|------|------|------|
| id | UUID | PK, default=uuid4 | ID |
| school_id | UUID | FK(users.id), UNIQUE, INDEX | 対象の学校ユーザー |
| early_check_enabled | BOOLEAN | NOT NULL, default=false | 早期チェック。ONの学校のみ締め日前確認メールを送る |
| notice_days_before | INTEGER | NOT NULL, default=3 | 締め日の何日前に送るか（0〜60・学校ごとに設定） |
| created_at / updated_at | TIMESTAMP WITH TZ | NOT NULL | 作成・更新日時 |

### work_school_deadlines（学校×対象月の締め日・202607161140）

年間分を月単位で設定する締め日（new側 migration 0018）。早期チェックONの学校は「締め日−N日〜締め日当日」の窓で1回だけ営業全員へ【至急確認】メールが送られる（`school_deadline_service`）。

| カラム | 型 | 制約 | 説明 |
|--------|------|------|------|
| id | UUID | PK, default=uuid4 | ID |
| school_id | UUID | FK(users.id), INDEX | 対象の学校ユーザー |
| target_month | VARCHAR(7) | INDEX | 対象月（YYYY-MM）。`UNIQUE(school_id, target_month)` |
| deadline_date | DATE | NOT NULL | 締め日（**対象月内の日付のみ**・202607161332） |
| notice_sent_at | TIMESTAMP WITH TZ | NULL | 確認メール送信済みガード（締め日変更で NULL に戻り再送対象） |
| created_at / updated_at | TIMESTAMP WITH TZ | NOT NULL | 作成・更新日時 |
