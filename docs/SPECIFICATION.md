...
- 報告書の作成は **当月分のみ** に限定される（既に最終承認済みの月への追加不可）
...
保護者差戻し（parent_return）
  ...
  ※ 保護者が差戻し後、取消（cancel）も可能

受付担当差戻し（return_from_receiver）
  submitted_to_admin または received --> returned_to_tutor

再鑑者差戻し（return_from_reviewer）
  received または re_reviewed --> returned_to_tutor

管理者差戻し（return_from_master）
  re_reviewed または admin_approved --> returned_to_tutor
...
| `admin_approved` | 最終承認済み | 管理者が最終承認。月次確定 |
| `returned_to_tutor` | 差戻し（要修正） | いずれかのフェーズで差戻し。講師が修正対応 |
...
[awaiting_parent_approval]                                              |
 保護者承認待ち                                                          |
  ...
  [submitted_to_admin]                                                  |
  受付待ち                                                               |
            | receive (admin_receiver)  return_from_receiver --------> |
            v                                                           |
       [received]                                                       |
       受付済み                                                          |
            | re_review (admin_reviewer)  return_from_reviewer ------> |
            v                                                           |
      [re_reviewed]                                                     |
      再鑑済み                                                           |
            | admin_approve (admin_master)  return_from_master ------> |
            v                                                           |
    [admin_approved] ----------- return_from_master (admin_master) --> +
    最終承認済み

差戻し先: [returned_to_tutor] --> submit_to_parent --> [awaiting_parent_approval]
```

### 各ステータスで可能なアクション

| ステータス | 可能なアクション | 実行ロール |
|-----------|----------------|-----------|
...
| re_reviewed | admin_approve, return_from_master | admin_master |
| re_reviewed | return_from_reviewer | admin_reviewer, admin_master |
| admin_approved | return_from_master | admin_master |
| returned_to_tutor | submit_to_parent, 編集 | tutor |
...
users
 ...
 |           (assignment_id)
 |                    v
 +-(tutor_id, parent_id)--> lesson_reports
                                   |
                    +--------------+--------------+
                    v              v               v
