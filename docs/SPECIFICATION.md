...
| `/admin/users` | ユーザー管理 | admin_master | 招待メール送信、未登録招待と登録済みユーザーの統合一覧、有効化・無効化、保護者詳細での「保護者承認スキップ」設定 |

> 旧「システム設定」画面（`/admin/assignments`）は廃止。保護者承認スキップは `/admin/users` の保護者詳細ドロワーに移設、リマインドは講師の承認管理画面（`/tutor/approval`）で承認依頼時に設定する。担当紐付けは講師の生徒管理（`/tutor/students`）と保護者招待（`/admin/users`）で作成する。
...
| skip_parent_approval | BOOLEAN | NOT NULL, default=False | 承認スキップ。保護者ユーザー＝保護者承認スキップ、学校ユーザー＝学校承認スキップ。ユーザー管理画面で設定 ※後から追加（migration 0013） |
...
| is_active | BOOLEAN | NOT NULL, default=True | 有効フラグ |
| skip_parent_approval | BOOLEAN | NOT NULL, default=False | （旧）承認スキップ。**現在は未使用**。スキップ判定は `users.skip_parent_approval`（保護者／学校ユーザー単位）に移設済み |
...
| `BASE_URL` | `http://localhost:8000` | O | 旧システム（指導実績報告 / backend）の公開 URL。招待・通知メールの URL 生成に使用 |
| `NEW_BASE_URL` | `http://localhost:8001` | O | 新システム（業務連絡表 / new_backend）の公開 URL。招待・通知メールの URL 生成に使用。旧システムと `.env` を共有するため `BASE_URL` とは別名 |
...
| `admin_master` | 経理（管理者） | 最終承認。完了後の修正依頼（差戻し）。ユーザー管理（学校承認スキップ設定を含む）・契約管理 |
...
- ユーザー管理・契約管理・招待は `admin_master` のみ（一部 `office` が profiles の作成・更新に参加）。学校承認スキップは `/admin/users` の学校詳細で設定する。担当紐付けは契約管理および講師の業務連絡表作成時に自動生成される（旧「システム設定」画面は廃止）。
...
学校承認が不要な学校（`users.skip_parent_approval = True`：学校ユーザー単位、経理のユーザー管理画面で設定）では、提出時に学校確認を飛ばして事務確認へ進む。手動でも `skip_school`（sales / office / admin_master）で `draft → awaiting_office` に進められる。
...
| `GET /admin/users` | admin/users.html | admin_master | ユーザー管理（招待統合、学校詳細で学校承認スキップ設定） |
