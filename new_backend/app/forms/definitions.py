...
monthly_dispatch = _register(FormDefinition(
    ...
    columns=(
        ...
        LineColumn(key="teach_minutes",  label="指導分",       type="number", summable=True),
        LineColumn(key="break_minutes",  label="休憩分",       type="number", summable=True),
        LineColumn(key="commute_fee",    label="往復交通費",   type="number", summable=True),
