...
def _run(report_id: UUID, action: str, payload: CommentIn, db: Session, user: User):
    report = get_report_for_user(report_id, user, db)
    transition(db, report, user, action, payload.comment)
    db.commit()
    db.refresh(report)
    send_transition_notifications(db, action, [report], user, payload.comment)
    return report
...
def submit_to_parent(report_id: UUID, payload: CommentIn = CommentIn(), db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    return _run(report_id, ReportAction.submit_to_parent.value, payload, db, user)
...
def parent_return(report_id: UUID, payload: CommentIn, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    if not payload.comment or not payload.comment.strip():
        raise HTTPException(status_code=422, detail="comment is required")
    payload.comment = payload.comment.strip()
    return _run(report_id, ReportAction.parent_return.value, payload, db, user)


@router.post("/{report_id}/submit-to-admin", response_model=ReportOut)
def submit_to_admin(report_id: UUID, payload: CommentIn = CommentIn(), db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    return _run(report_id, ReportAction.submit_to_admin.value, payload, db, user)


@router.post("/submit-to-admin-bulk")
def submit_to_admin_bulk(payload: BulkSubmitIn, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
def submit_to_parent_bulk(payload: BulkSubmitIn, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    reports = _bulk_reports(payload, db, user)
    ...
    for report in reports:
        ...
        changed.append(report.id)
    db.commit()
    send_transition_notifications(db, ReportAction.submit_to_parent.value, reports, user)
    return {"updated": changed}


@router.post("/parent-approve-bulk")
def parent_approve_bulk(payload: BulkSubmitIn, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    reports = _bulk_reports(payload, db, user)
...
def parent_return_bulk(payload: BulkReturnIn, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    reports = _bulk_reports(payload, db, user)
    _validate_bulk(reports, user_id=user.id, owner_attr="parent_id", status=ReportStatus.awaiting_parent_approval.value)
    _validate_target_month(reports, payload.target_month)
    changed = []
    for report in reports:
        ...
        changed.append(report.id)
    db.commit()
    send_transition_notifications(db, ReportAction.parent_return.value, reports, user, payload.comment)
    return {"updated": changed}


@router.post("/admin-return-bulk")
def admin_return_bulk(payload: AdminBulkReturnIn, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    action_by_role = {
    ...
    for report in reports:
        ...
        changed.append(report.id)
    db.commit()
    send_transition_notifications(db, action, reports, user, payload.comment)
    return {"updated": changed}


@router.post("/admin-receive-bulk")
def admin_receive_bulk(payload: BulkSubmitIn, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
def admin_review_bulk(payload: BulkSubmitIn, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
def admin_approve_bulk(payload: BulkSubmitIn, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    for report in reports:
        ...
        changed.append(report.id)
    db.commit()
    send_transition_notifications(db, ReportAction.admin_approve.value, reports, user)
    return {"updated": changed}
...
def receive(report_id: UUID, payload: CommentIn = CommentIn(), db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    return _run(report_id, ReportAction.receive.value, payload, db, user)


@router.post("/{report_id}/return-from-receiver", response_model=ReportOut)
def return_from_receiver(report_id: UUID, payload: CommentIn, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    return _run(report_id, ReportAction.return_from_receiver.value, payload, db, user)


@router.post("/{report_id}/re-review", response_model=ReportOut)
def re_review(report_id: UUID, payload: CommentIn = CommentIn(), db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    return _run(report_id, ReportAction.re_review.value, payload, db, user)


@router.post("/{report_id}/return-from-reviewer", response_model=ReportOut)
def return_from_reviewer(report_id: UUID, payload: CommentIn, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    return _run(report_id, ReportAction.return_from_reviewer.value, payload, db, user)


@router.post("/{report_id}/admin-approve", response_model=ReportOut)
def admin_approve(report_id: UUID, payload: CommentIn = CommentIn(), db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    return _run(report_id, ReportAction.admin_approve.value, payload, db, user)


@router.post("/{report_id}/return-from-master", response_model=ReportOut)
def return_from_master(report_id: UUID, payload: CommentIn, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    return _run(report_id, ReportAction.return_from_master.value, payload, db, user)
# === Phase 5 END ===
